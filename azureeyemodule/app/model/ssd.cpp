...
SSDModel::SSDModel(const std::string &labelfpath, const std::vector<std::string> &modelfpaths, const std::string &mvcmd, const std::string inputsource, const std::string &videofile, const cv::gapi::mx::Camera::Mode &resolution, bool show)
...
cv::GStreamingCompiled SSDModel::compile_cv_graph() const
{
    ...
    //    graph = cv::GComputation(cv::GIn(in),
    ...
    cv::GArray<int> tags;
        ...
        auto graph = cv::GComputation(cv::GIn(in), cv::GOut(objs, tags));
    //}
